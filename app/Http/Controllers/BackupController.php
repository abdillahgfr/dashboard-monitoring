...
class HomeController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $getSPPBData = function ($connection, $columnAlias, $filter, $currentMonth) {
            return DB::connection($connection)
                ->table('rq_data2025')
                ->select('idskpd', DB::raw("COUNT(*) AS $columnAlias"))
                ->whereRaw("SUBSTRING(LTRIM(RTRIM(noref)), 8, 5) = '$filter'")
                ->whereYear('tgl_rq', '2025')
                ->whereMonth('tgl_rq', $currentMonth) // Filter for April 2025
        ...
        $sppb1 = $getSPPBData('sqlsrv_2', 'SPPB1', 'RQ2.1', $currentMonth);
        $sppb2 = $getSPPBData('sqlsrv_2', 'SPPB2', 'RQ2.3', $currentMonth);
        $sppb3 = $getSPPBData('sqlsrv_2', 'SPPB3', 'RQ2.4', $currentMonth);
        $sppb4 = $getSPPBData('sqlsrv_2', 'SPPB4', 'RQ2.5', $currentMonth);

        // Helper function for fetching BASTSPPB data from rq_data2025
        $getBASTSPPBData = function ($connection, $columnAlias, $filter, $currentMonth) {
            return DB::connection($connection)
                ->table('rq_data2025')
                ->select('idskpd', DB::raw("COUNT(*) AS $columnAlias"))
                ->whereRaw("SUBSTRING(LTRIM(RTRIM(noref)), 8, 5) = '$filter'")
        ...
        $bastSPPB1 = $getBASTSPPBData('sqlsrv_2', 'BASTSPPB1', 'RQ2.1', $currentMonth);
        $bastSPPB2 = $getBASTSPPBData('sqlsrv_2', 'BASTSPPB2', 'RQ2.3', $currentMonth);
        $bastSPPB3 = $getBASTSPPBData('sqlsrv_2', 'BASTSPPB3', 'RQ2.4', $currentMonth);
        $bastSPPB4 = $getBASTSPPBData('sqlsrv_2', 'BASTSPPB4', 'RQ2.5', $currentMonth);

        // Helper function for fetching BASTPHK3 and BASTHIBAH from bast_data2025
        $getBASTData = function ($connection, $columnAlias, $filter, $currentMonth) {
            return DB::connection($connection)
                ->table('bast_data2025')
                ->select('kolok', DB::raw("COUNT(*) AS $columnAlias"))
                ->where('tipe_bast', $filter)
                ->whereYear('tgl_bast', '2025')
                ->whereMonth('tgl_bast', $currentMonth) // Filter for April 2025
                ->where(function ($query) {
        ...
        $bastPHK3 = $getBASTData('sqlsrv_2', 'BASTPHK3', '1', $currentMonth);
        $bastHIBAH = $getBASTData('sqlsrv_2', 'BASTHIBAH', '2', $currentMonth);
        ...
        $getTambahData = function ($connection, $columnAlias, $filter, $currentMonth) {
            return DB::connection($connection)
                ->table('bast_data2025')
                ->select('kolok', DB::raw("COUNT(*) AS $columnAlias"))
                ->where('tipe_bast', $filter)
        ...
        $getKurangData = function ($connection, $columnAlias, $filter, $currentMonth) {
            return DB::connection($connection)
                ->table('rq_data2025')
        ...
        $reviewTambah = $getTambahData('sqlsrv_2', 'REVIEWTAMBAH', '7', $currentMonth);
        $instTambah = $getTambahData('sqlsrv_2', 'INSTAMBAH', '5', $currentMonth);
        $auditTambah = $getTambahData('sqlsrv_2', 'AUDITTAMBAH', '6', $currentMonth);

        // Retrieve "Kurang" data
        $reviewKurang = $getKurangData('sqlsrv_2', 'REVIEWKURANG', '4.6', $currentMonth);
        $insKurang = $getKurangData('sqlsrv_2', 'INSKURANG', '4.2', $currentMonth);
        $auditKurang = $getKurangData('sqlsrv_2', 'AUDITKURANG', '4.4', $currentMonth);
